...
#===================================
def communityDetection_spectral_BH(A, r, classifier='Kmeans', qWanted=-1):
    '''Look for qWanted community in the graph G using the Bethe-Hessian method

    [+] A: scipy sparse matrix
            Adjacency matrix (supposed sparse)
    [+] classifier: string
            Method used for the classification (Kmeans or GMM)

    [-] membership: (len(A),) numpy array
            Vector that give an assignment to each nodes according its community
    [-] nCommunities: int
            Number of communities returned by the algorithm
    '''

    if classifier in ["Kmeans", "GMM", "BGMM"]:
        print("Starting BH community detection with " + classifier + " classifier")
    else:
        raise Exception("BH community detection: Unkown classification method asked, exit")

    time_start = time.time()

    n = A.shape[0]     #Number of nodes
    qMax = 2*np.floor(np.sqrt(n))        #Nb max of communities to look for
    qMax = qMax.astype('int64')

    H = matrix_BetheHessian(A, r)      #Compute BH
    eigvals, eigvects = sparse.linalg.eigsh(H, k=qMax, which='SA')      #smaller algebraic values

    #Plot spectral density
    plt.figure()
    plt.hist(eigvals, color="#3F5D7D", edgeColor="black", lineWidth=0.2, bins=25, log=True)
    plt.title('Spectral density of H with r=' + utility.nice_looking_str(r) + ' and qMax = ' + str(qMax))

    q = len(eigvals[eigvals<0])     #Ncommunities are nb of eig < 0
    if qWanted>0 and qWanted < q:
        q = qWanted

    indices = eigvals.argsort()[:q]     #Get indices of negative eigenvalues
    W = eigvects[:, indices]        #Only keep vectors with these eigenvalues

    #Normalizing all eigenvectors to 1 if not done already
    #Here it is already done
    for i in range(q):
        W[:, i] = W[:, i] / np.linalg.norm(W[:, i])


    #Clustering of communities in the eingen space
    nCommunities = q
    if classifier == "Kmeans":
        classif = KMeans(q).fit(W)       #Use the K-means algo to classify entries
        membership = classif.labels_
    elif classifier == "GMM":
        alg = GaussianMixture(q).fit(W)
        membership = alg.predict(W)
    elif classifier == "BGMM":
        alg = BayesianGaussianMixture(q, weight_concentration_prior=1e-2).fit(W)
        membership = alg.predict(W)

    print('BH clustering done! Time elapsed: {} seconds'.format(time.time()-time_start))
    return membership, nCommunities


def studyCommunityDetection(A, rIt, nIterGmm=5):
    n = len(A)
    qMax = 2*np.floor(np.sqrt(n))        #Nb max of communities to look for
    qMax = qMax.astype('int64')

    print("Starting to study behavior of BH clustering")
    ...
    for r in rIt:
        ...
        #Perform GMM clustering on induced W space
        indices = eigvals.argsort()[:q]     #Get indices of negative eigenvalues
        W = eigvects[:, indices]        #Only keep vectors with these eigenvalues
        bicTmp = 0
        logLikeTmp = 0
        for i in range(nIterGmm):
            alg = GaussianMixture(q).fit(W)
            membershipTmp = alg.predict(W)
            bicTmp += alg.bic(W)/nIterGmm
            logLikeTmp += alg.score(W)/nIterGmm

        #Store values
        logLike.append(logLikeTmp)
        bic.append(bicTmp)
        membership.append(membershipTmp)

        print('Iteration ' + str(len(vecCom)) + '/' + str(len(rIt)) +
              ' -- Time elapsed: {} seconds'.format(time.time()-t_deb))

    bic = np.asarray(bic)
    logLike = np.asarray(logLike)
    vecCom = np.asarray(vecCom)

    #========== Plot resutls
    fig, ax = plt.subplots(2,1, figsize=(12, 8))
    fig.subplots_adjust(hspace=.4)
    ax[0].set_title('#Com = f(r)')
    ax[0].plot(rIt, vecCom, 'o-')
    ax[0].set_yscale('log', nonposy='clip')
    ax[0].set_xlabel('r')
    ax[0].set_ylabel('#com')

    ax[1].set_title('Loglike = f(r)')
    ax[1].plot(rIt, logLike, 'o-')
    ax[1].set_xlabel('r')
    ax[1].set_ylabel('Loglike')
    ax[1].set_yscale('log')

    ax2 = ax[1].twinx()  # instantiate a second axes that shares the same x-axis
    ax2.set_ylabel('bic', color='red')
    ax2.plot(rIt, bic, 'o-', color='red')
    ax2.tick_params(axis='y', labelcolor='red')
    fig.savefig('Images/study_BH_detection.png')

    return vecCom, bic, logLike, membership



def iterate_SLP(lam0, A, errtol=1e-8, maxIter=10):
    '''Iterates SLP starting from lam0 and tries to find the closest nonlinear eigenvalues
[+] lam0: first guess as starting point to solve the eigenvalue problem
[+] A: adjacency matrix
[+] errtol: tolerated error
[+] maxIter: nb max of iteration allow to solve the problem
[-] x: eigenvector of the solution
[-] lam: eigenvalue that solves the problem'''

    print("SLP algorithm to solve iteratively nonlinear eigenvalue problem")
    time_start = time.time()
    err=1
    it=0
    lam = lam0
    ...
    while err > errtol and it < maxIter:
        H = matrix_BetheHessian(A, lam)
        H_prime = matrix_BetheHessian_prime(A, lam)
        mu, x = sparse.linalg.eigsh(H, M=H_prime, k=1, which='SM')
        err = abs(np.asscalar(mu))
        lam = lam - np.asscalar(mu)
        it += 1
        print("Iteration #" + str(it) + " -- lam=" + str(lam) + " -- error=" + str(err))
        vecLam.append(lam)
        vecErr.append(err)

    print('SLP done! Time elapsed: {} seconds'.format(time.time()-time_start))
    return lam, x
...
#  Matrix computation
#==========================
def matrix_BetheHessian(A, r=None):
    '''Compute the BH matrix given an adjacency matrix and a value for r, the regularization factor

    [+] A: (N,N) numpy array
            Adjacency matrix
    [+] r: float
            Regularization factor for the BH, classical Laplacian corresponds to r=1

    [-] H: numpy array
            Associated Bethe Hessian matrix
    '''

    d = np.array(np.sum(A, axis=0))
    if r is None:
        r = np.sqrt(np.mean(d**2) / np.mean(d) - 1)

    D = np.diag(d)      #Degree matrix
    n = max(d.shape)
    H = (r*r - 1) * sparse.eye(n) - r*A + D
    return H



def matrix_BetheHessian_prime(A, r=None):
    '''Compute the BH prime matrix given an adjacency matrix and a value for r, the regularization factor
    ...
    [-] H: numpy array
            Associated Bethe Hessian prime matrix
    '''

    n = A.shape[0]
    H_prime = 2*r*sparse.eye(n) - A
    return H_prime
...
#    edgesInLayer = []
    ...
    #======Plot the onion spectrum
    #Plot the global onion spectrum per cores
    if flagFig == 1:
        fig, ax = utility.getFigure(figSize, axSize) #plt.figure(figsize=(10,8))
        fraction = []
        n1 = 1
        n2 = n1
        for k in kcores:        #For each core
            for l in k:         #For each layer of the core
                fraction.append(len(l) / len(d))
            n2 += len(k)
            plt.plot(range(n1, n2), fraction, 'o-', color='navy')
            n1 = n2
#            fraction = []

        #TO PLOT IN THE ARTICLE
#        plt.axvline(x=14, color='k', linestyle='-', alpha=0.7)
#        plt.axvline(x=39, color='k', linestyle='--', alpha=0.7)

        ax.set_title('Onion spectrum of the graph')
        plt.yscale('log')
        ax.set_xlabel('Layer')
...
#        diameters = []
#        if len(edges_span) > 0:
#            for l in range(0, int(max(layer))):
#                idx = np.where(layer > l)[0]
#                edges_pruned = [e for (i,e) in enumerate(edges_span) if (e[0] in idx and e[1] in idx)]
#                G_pruned = ig.Graph(edges_pruned, directed=False)
##                ecc = np.array(G_pruned.eccentricity())
#                diam = G_pruned.diameter()
#                diameters.append(diam)
#
#            plt.figure(figsize=(10,8))
#            plt.plot(range(0, int(max(layer))), diameters, 'o-')
#            plt.title('Diameter versus layer')
#            plt.xlabel('Layer')
#            plt.ylabel('Diameter')
