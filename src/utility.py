...
#Optimal number of bins for histogram plots
def nBins_Freedman_Diaconis(x):
    if type(x) is not np.ndarray:
        raise Exception("Wrong type: x should be a numpy.ndarray")

    h = sp_st.iqr(x)*2 / len(x)**(1./3)        #Freedmann-Diaconis rule gives a width for histogram bars
    if h > 0:
        optNbins = int(1.*(max(x) - min(x)) / h)
    else:
        warnings.warn("h <= 0, check your data (Possibly IQR is 0)")
        if len(x) > 30:
            warnings.warn("Use Sturges' formula instead")
            optNbins = np.log2(len(x))+1
        else:
            raise warnings.warn("Use Rice rule instead")
            optNbins = 2*len(x)**(1/3)

    return optNbins


#Thanks to https://stackoverflow.com/questions/22562364/circular-histogram-for-python
def rose_plot(ax, angles, bins=16, density=None, offset=0, color='C0', lab_unit="degrees",
    ...
    ax.bar(bin[:-1], radius, zorder=1, align='edge', width=widths,
           edgecolor=color, fill=True, alpha=0.5, linewidth=1)
...
def cloudDiff(data1, data2, R):
    '''
Compute the difference between two cloud points:
[-] data1: first cloud points coord (N1xD) with N1 points and D dimensions
[-] data2: second cloud points coord (N2xD) with N2 points and D dimensions
[-] R: the radius around each data points

We return a set of indices from data2 that ARE NOT found in data1 and a set of indices from data1 that are not found in data2
    '''
    idxData2NotInData1 = []
    idxData1NotInData2 = []
    for i in range(len(data2)):
        dist = np.linalg.norm(data2[i] - data1, axis=1)
        minDist = np.min(dist)
        if minDist > R:  #Then the point of data2 is NOT found in data1 under the radius R
            idxData2NotInData1.append(i)

    for i in range(len(data1)):
        dist = np.linalg.norm(data1[i] - data2, axis=1)
        minDist = np.min(dist)
        if minDist > R:
            idxData1NotInData2.append(i)

    idxData1NotInData2 = np.asarray(idxData1NotInData2)
    idxData2NotInData1 = np.asarray(idxData2NotInData1)

    return idxData1NotInData2, idxData2NotInData1
